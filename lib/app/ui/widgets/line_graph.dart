...
class _LineGraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final linePaint = Paint()..color = lineColor..strokeWidth = 2..style = PaintingStyle.stroke;
    ...
    for (int i = 0; i <= horizontalGridLines; i++) {
      ...
      final valueLabel = ((horizontalGridLines - i) * maxValue / horizontalGridLines).round().toString();
      textPainter.text = TextSpan(text: valueLabel, style: const TextStyle(fontSize: 10));
      textPainter.layout();
      textPainter.paint(canvas, Offset(left - textPainter.width - 4, dy - textPainter.height / 2));
    }
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      textPainter.text = TextSpan(text: data[i].label, style: const TextStyle(fontSize: 12));
      textPainter.layout();
